...
#---------------------------------------------------------------------------------
APP_TITLE		:=	LocalFileshare
APP_DESCRIPTION	:=	Transfer files between consoles through infrared
...
OUTDIR		:=	out
BUILD		:=	build
SOURCES		:=	source source/app source/states
DATA		:=	data
INCLUDES	:=	include include/app include/states
GRAPHICS	:=	gfx
ROMFS		:=	romfs
GFXBUILD	:=	$(ROMFS)/gfx
...
LIBS	:= -lcitro2d -lcitro3d -lctru -lstdc++fs -lm
